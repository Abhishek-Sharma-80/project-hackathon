...
export const SkillTestPage: React.FC = () => {
  ...
  if (loading) {
    return <LoadingScreen message={`Preparing verified assessment test for ${skillName}...`} />;
  }
    ...
    return (
      <div className="space-y-8 animate-fadeIn max-w-3xl mx-auto py-6">
        <div className="p-8 sm:p-10 rounded-3xl bg-white border border-slate-200 shadow-xl text-center space-y-6">
          ...
          <div className="text-left p-5 rounded-2xl bg-slate-50 border border-slate-100 space-y-2">
            <span className="text-xs font-bold text-slate-800 uppercase tracking-wider block">
              Recommended Next Steps:
            </span>
            <ul className="space-y-1 text-xs text-slate-600">
              {result.nextRecommendations?.map((rec: string) => (
                <li key={rec} className="flex items-center gap-1.5 font-medium">
