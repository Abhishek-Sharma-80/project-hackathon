...
export const LearningModulePage: React.FC = () => {
  ...
  const fetchModule = async () => {
    try {
      ...
      const path = res.data.learningPath;
  ...
  const handleCompleteLesson = async () => {
    ...
    try {
      ...
      setNotification(`🎉 Lesson completed! Your skills in ${res.data.improvedSkills?.join(', ')} were upgraded, boosting your Profile Score & Internship Matches!`);
  ...
  if (loading) {
    return <LoadingScreen message="Loading interactive lesson environment and code challenges..." />;
  }
  ...
  return (
    <div className="space-y-6 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Right: Active Lesson Content & Interactive Code Sandbox */}
          ...
          <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200 shadow-sm space-y-6">
            ...
            {activeLesson?.resources && (
              <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-100">
                <span className="text-xs font-bold text-indigo-900 block mb-1">Recommended Industry Docs:</span>
                <p className="text-xs text-indigo-700">{activeLesson.resources}</p>
              </div>
            )}

            {/* Bottom Controls: Previous / Mark Complete / Next */}
