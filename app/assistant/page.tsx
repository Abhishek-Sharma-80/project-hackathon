...
import { Sparkles, Send, Bot, User, CornerDownLeft, Zap } from 'lucide-react';
...
export default function AIAssistantPage() {
  ...
  const handleSend = async (msgText: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { sender: 'AI', message: res.reply }]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        ...
        <div className="p-4 sm:p-5 border-b border-slate-100 bg-slate-50/80 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">Groq LLaMA 3.3 70B Versatile • Grounded in your real database profile</p>
            </div>
          </div>

          <div className="text-xs font-semibold text-purple-700 bg-purple-50 px-3 py-1 rounded-full border border-purple-100">
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 p-2">
              ...
              <span>Groq LLaMA 3.3 70B is analyzing your profile context...</span>
            </div>
          )}
