...
async function main() {
  console.log('🌱 Seeding SmartEdu AI Database...');
  ...
  const nexus = companyMap.get('Nexus Labs');
  const innovatex = companyMap.get('InnovateX Technologies');
  const brightlearn = companyMap.get('BrightLearn AI');
  const hyperscale = companyMap.get('HyperScale Cloud');
  const apex = companyMap.get('Apex Mobility');

  const internshipsData = [
    // PRIMARY RECOMMENDATION FOR ARYAN (91% Match)
    {
  ...
  // Aryan's Certifications (6 certifications)
  ...
  // Aryan's Applications (12 applications across statuses)
  ...
  // 6. Create Assessment Questions for Skills (Java, Python, SQL, DSA, React, Spring Boot, AWS)
  const assessmentsQuestionsData = [
    // JAVA
    {
      ...
      questions: [
        ...
        {
          question: 'What is the time complexity of searching a key in a balanced Java HashMap under optimal conditions?',
          options: JSON.stringify(['O(1) average time', 'O(log N)', 'O(N)', 'O(N^2)']),
          correctIndex: 0,
          explanation: 'HashMap lookup achieves O(1) average time complexity via bucket hashing. (O(log N) worst case when treeified under Java 8).',
          difficulty: 'EASY',
        },
      ],
    },
    // SQL
    {
      ...
      questions: [
        ...
        {
          question: 'Which SQL clause is evaluated BEFORE the GROUP BY clause during query execution?',
          options: JSON.stringify(['HAVING', 'WHERE', 'ORDER BY', 'SELECT']),
          correctIndex: 1,
          explanation: 'In SQL execution order: FROM -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY.',
          difficulty: 'EASY',
        },
      ],
    },
    // SPRING BOOT
    {
      ...
      questions: [
        ...
        {
          question: 'Which annotation in Spring Boot is used to create a RESTful endpoint returning JSON responses directly?',
          options: JSON.stringify(['@RestController', '@Controller', '@ResponseBodyOnly', '@Service']),
          correctIndex: 0,
          explanation: '@RestController combines @Controller and @ResponseBody so returned objects are serialized automatically to JSON.',
          difficulty: 'EASY',
        },
      ],
    },
  ...
  const dummyStudents = [
    ...
    { name: 'Tanvi Joshi', email: 'tanvi.j@galgotias.edu', cgpa: 6.9, branch: 'Information Technology', role: 'Frontend Developer', score: 58, readiness: 52 }, // At risk
  ...
  console.log('👤 Demo Student: aryan@smartedu.ai / Aryan@123');
  console.log('🛡️ Admin: admin@smartedu.ai / Admin@123');
}
