...
exports.learningPathService = exports.LearningPathService = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const profileScoreEngine_1 = require("./profileScoreEngine");
class LearningPathService {
    /**
     * Generates or retrieves personalized learning path for student
     */
    async getOrCreateLearningPath(studentId, careerPath = 'Backend Developer') {
        let learningPath = await prisma_1.default.learningPath.findFirst({
            where: { studentId },
            include: {
                modules: {
                    orderBy: { order: 'asc' },
    ...
    /**
     * Creates structured default roadmap with lessons for backend developer or target path
     */
    async seedDefaultRoadmap(studentId, careerPath) {
        const newPath = await prisma_1.default.learningPath.create({
            data: {
                studentId,
                careerPath,
                overallProgress: 18,
                        ...
                        },
                        {
                            ...
                            lessons: {
                                create: [
                                    ...
                                    {
                                        title: '2.3 OpenAPI 3.0 & Swagger UI Integration',
                                        duration: '35 mins',
                                        completed: false,
                                        order: 3,
                                        summary: 'Auto-generate interactive API documentation and testing UI.',
                                        codeSnippet: '@Operation(summary = "Get explainable recommendation")\n@ApiResponse(responseCode = "200", description = "Match generated")',
                                        resources: 'OpenAPI Specification v3.0',
                                    },
                                ],
                            },
                        },
                        {
                            ...
                            lessons: {
                                create: [
                                    ...
                                    {
                                        title: '3.2 Multi-Container Orchestration with Docker Compose',
                                        duration: '50 mins',
                                        completed: false,
                                        order: 2,
                                        summary: 'Spin up backend app, PostgreSQL database, and Redis cache in isolated bridge networks.',
                                        codeSnippet: 'services:\n  backend:\n    build: .\n    ports: ["8080:8080"]\n    depends_on: [postgres, redis]\n  postgres:\n    image: postgres:16-alpine',
                                        resources: 'Docker Compose V2 Documentation',
                                    },
                                ],
                            },
                        },
                        {
                            ...
                            lessons: {
                                create: [
                                    ...
                                    {
                                        title: '4.2 End-to-End Implementation & Testing',
                                        duration: '120 mins',
                                        completed: false,
                                        order: 2,
                                        summary: 'Implement business logic, unit tests with Mockito, and integration tests with Testcontainers.',
                                        codeSnippet: '@Test\nvoid shouldReturnMatchScoreAbove90Percent() {\n  // Assertion\n}',
                                        resources: 'Testcontainers Java Guide',
                                    },
                                ],
                            },
                        },
    ...
    /**
     * Completes a lesson, recalculates module progress, upgrades student skills, and updates profile & recommendations
     */
    async completeLesson(lessonId, studentId) {
        // 1. Mark lesson as completed
        const lesson = await prisma_1.default.learningLesson.update({
            where: { id: lessonId },
            data: { completed: true },
        ...
        await prisma_1.default.learningModule.update({
        ...
        // If module finished, unlock next module
        if (isModuleFinished) {
            const nextModule = await prisma_1.default.learningModule.findFirst({
                where: {
        ...
        // 2. Dynamically upgrade student skills improved by this module
        const skillNames = moduleRecord.skillsImproved.split(',').map((s) => s.trim());
        for (const skillName of skillNames) {
            const skill = await prisma_1.default.skill.findUnique({ where: { name: skillName } });
            if (skill) {
                const studentSkill = await prisma_1.default.studentSkill.findUnique({
                    where: {
                        studentId_skillId: {
                            studentId,
                            skillId: skill.id,
        ...
        // 3. Recalculate student profile score
        ...
        if (student) {
            const breakdown = (0, profileScoreEngine_1.calculateProfileScore)(student);
        ...
        // 4. Update overall learning path progress
        const allModules = await prisma_1.default.learningModule.findMany({
            where: { learningPathId: moduleRecord.learningPathId },
        });
        const avgProgress = Math.round(allModules.reduce((acc, m) => acc + (m.id === moduleRecord.id ? moduleProgress : m.progress), 0) /
            allModules.length);
        await prisma_1.default.learningPath.update({
...
exports.learningPathService = new LearningPathService();
