...
exports.getReports = exports.createCompany = exports.getCompanies = exports.deleteInternship = exports.updateInternship = exports.createInternship = exports.getStudentById = exports.getStudentsList = exports.getInterventionStudents = exports.getAdminAnalytics = void 0;
const prisma_1 = __importDefault(require("../config/prisma"));
const getAdminAnalytics = async (req, res) => {
    try {
        const totalStudents = await prisma_1.default.studentProfile.count();
        const totalInternships = await prisma_1.default.internship.count();
        ...
        const internshipSkills = await prisma_1.default.internshipSkill.findMany({
            include: { skill: true },
        });
        const skillCounts = {};
        ...
        const mostDemandedSkills = Object.entries(skillCounts)
            ...
            .slice(0, 8);
        // Application funnel
        const apps = await prisma_1.default.application.findMany();
        const funnel = {
...
exports.getInterventionStudents = getInterventionStudents;
const getStudentsList = async (req, res) => {
    try {
        const { search, branch, targetRole } = req.query;
        const where = {};
        if (branch && branch !== 'all')
            where.branch = { contains: String(branch) };
        if (targetRole && targetRole !== 'all')
            where.targetRole = { contains: String(targetRole) };
        const students = await prisma_1.default.studentProfile.findMany({
            where,
            include: {
                user: true,
        ...
        if (search) {
            ...
            filtered = students.filter((s) => s.user.name.toLowerCase().includes(q) ||
...
const createInternship = async (req, res) => {
    try {
        const { title, companyId, companyName, description, location, workMode, stipend, duration, deadline, careerCategory, responsibilities, eligibility, skills = [], // [{ name, requiredLevel, importance }]
         } = req.body;
        let targetCompanyId = companyId;
        if (!targetCompanyId && companyName) {
            let company = await prisma_1.default.company.findFirst({ where: { name: companyName } });
            if (!company) {
        ...
        const internship = await prisma_1.default.internship.create({
            data: {
                title,
        ...
        for (const s of skills) {
            ...
            await prisma_1.default.internshipSkill.create({
                data: {
        ...
        const fullInternship = await prisma_1.default.internship.findUnique({
...
const updateInternship = async (req, res) => {
    try {
        ...
        const internship = await prisma_1.default.internship.update({
            where: { id },
            data: {
                title,
                description,
...
const deleteInternship = async (req, res) => {
    try {
        const { id } = req.params;
        await prisma_1.default.internship.delete({ where: { id } });
