import { InternshipMatchResult, MatchFactorBreakdown } from '../types';
...
export class RecommendationEngine {
  ...
  public calculateMatch(studentProfile: any, internship: any): InternshipMatchResult {
    ...
    const projectTechs = projects.flatMap((p: any) => (p.technologies || '').toLowerCase().split(','));
    const matchedProjectTech = requiredSkillsList.filter((req: any) =>
      projectTechs.some((t: string) => t.trim() === (req.skill?.name || '').toLowerCase())
    );
    if (matchedProjectTech.length >= 2) projectRelevance = Math.min(100, projectRelevance + 10);
  ...
  private generateExplanation(
    ...
    if (matchScore >= 85) {
      return `Outstanding match! Your solid foundation in ${topMatches || 'core engineering'} directly satisfies ${internship.company?.name || 'this company'}'s primary requirements. ${
        topGaps ? `Improving ${topGaps} via your personalized learning roadmap will elevate your readiness from ${matchScore}% to ${potentialScore}%, making you a standout tier-1 applicant.` : 'You already meet or exceed all critical qualifications for this position.'
      }`;
