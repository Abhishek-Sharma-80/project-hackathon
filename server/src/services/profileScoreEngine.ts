export interface ProfileScoreBreakdown {
  ...
  projectScore: number;        // max 25
  certificationScore: number;  // max 15
  careerGoalScore: number;     // max 10
  assessmentScore: number;     // max 10
...
export function calculateProfileScore(profile: {
  cgpa?: number;
  skills?: { level: number; isVerified?: boolean }[];
  projects?: any[];
  certifications?: any[];
  careerGoals?: any[];
  assessments?: any[];
  ...
  let projectScore = 0;
  if (projectCount >= 4) projectScore = 25;
  ...
  if (certCount >= 4) certificationScore = 15;
  else if (certCount >= 2) certificationScore = 10;
  else if (certCount >= 1) certificationScore = 5;
  else {
  ...
  if (verifiedCount >= 3) assessmentScore = 10;
  else if (verifiedCount >= 1) assessmentScore = 5;
  else {
