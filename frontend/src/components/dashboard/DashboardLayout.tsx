        ...
        <div className="p-3 border-t border-slate-100 dark:border-slate-800/80">
          {user?.role === 'recruiter' || location.pathname === '/recruiter' ? (
            ...
            <div className={`flex items-center ${collapsed ? 'justify-center' : 'space-x-3'} p-2 rounded-2xl bg-slate-50 dark:bg-slate-800/60`}>
              {/* Dynamic Avatar: show image if available, else initials */}
              {user?.avatar ? (
                <img
                  src={user.avatar}
                  alt={user.name}
                ...
                <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-600 flex items-center justify-center text-white text-xs font-black shrink-0">
                  {(user?.name || profile?.fullName || 'U')
                    .split(' ')
                    .map((w: string) => w[0])
                    .join('')
                    .toUpperCase()
              ...
              {!collapsed && (
                <div className="truncate flex-1">
                  <p className="text-xs font-bold text-slate-900 dark:text-white truncate">{user?.name || profile?.fullName || 'Student'}</p>
                  <p className="text-[10px] text-slate-400 truncate">{user?.email || profile?.email || 'student@interndisha.edu'}</p>
                </div>
              )}
              {!collapsed && (
                <button
